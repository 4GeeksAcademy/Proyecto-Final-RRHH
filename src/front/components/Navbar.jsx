          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {userOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 z-50">
                {/* USER INFO */}
                <div className="p-4">
                  <p className="font-semibold text-black dark:text-white">{user?.nombre || "Cargando..."}</p>
                  ...
                  <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
                    {Object.keys(statusColors).map((s) => (
                      <li
                        key={s}
                        onClick={() => setStatus(s)}
                        className="flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer capitalize"
                      >
                ...
                <ul className="text-sm text-gray-700 dark:text-gray-300">
                  ...
                  {/* Dark mode */}
                  <li className="px-4 py-2 border-t border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <button
                      onClick={() => setDarkMode(!darkMode)}
                      className="flex items-center justify-between w-full text-sm font-medium text-black dark:text-white"
                    >
                      <div className="flex items-center gap-2">
                        {darkMode ? (
                          <i className="bi bi-brightness-high text-yellow-400"></i>
                        ) : (
                          <i className="bi bi-moon text-gray-700 dark:text-gray-300"></i>
                        )}
                        <span>{darkMode ? 'Modo Claro' : 'Modo Oscuro'}</span>
                      </div>
                      <div className={`relative inline-block w-10 h-6 rounded-full transition-colors ${darkMode ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
                        <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white dark:bg-gray-300 rounded-full shadow transform transition-transform ${darkMode ? 'translate-x-5' : 'translate-x-0'}`}></div>
                      </div>
                    </button>
                  </li>
                  
                  {/* IDIOMAS */}
                  <li className="relative border-t border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <button
                      onClick={() => setLangOpen(!langOpen)}
                      className="flex items-center justify-between w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm font-medium text-black dark:text-white"
                    >
                      <div className="flex items-center gap-2">
                      ...
                      <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
                        {languages.map((lang) => (
                          <button
                            ...
                            className="flex items-center w-full px-8 py-2 text-xs hover:bg-blue-50 dark:hover:bg-blue-900/20 text-left text-black dark:text-white"
                          >
                            {lang.name}
                          </button>
                        ))}
                  ...
                  {/* SIGN OUT */}
                  <li className="flex items-center gap-3 px-4 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 cursor-pointer">
          ...
          <div className="flex items-center">
            <a href="/" className="flex items-center mr-6">
              <img
                ...
                alt="Logo"
              />
              <span className="text-2xl font-semibold text-black dark:text-white">TeamCore</span>
            </a>
          </div>
