...
export default function Tareas({ onTareasUpdate }) {
  ...
  const cargarTareas = async () => {
    ...
    try {
      ...
      setTareas(data);
      // Sincronizar con el store global para que la gráfica se actualice
      actualizarTareasGlobales(data);
    } catch (err) {
  ...
  const handleAddTarea = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Error creando tarea");
      }

      const created = await res.json();

      setTareas((prev) => {
        const nuevasTareas = [...prev, created];
        actualizarTareasGlobales(nuevasTareas);
        return nuevasTareas;
      });
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    ...
    try {
      ...
      setTareas((prev) => {
        ...
        actualizarTareasGlobales(nuevas); // Importante: Sincronizar para mover la gráfica
        return nuevas;
      });
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      setTareas((prev) => {
        const nuevas = prev.map((t) => (t.id === tareaActualizada.id ? tareaActualizada : t));
        actualizarTareasGlobales(nuevas);
        return nuevas;
      });
